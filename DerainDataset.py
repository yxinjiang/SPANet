...
def normalize(data):
    return data / 255.
def resize(data,size=(200,200)):
    return cv2.resize(data,size)

def Im2Patch(img, win=100, stride=1):
    k = 0
    endc = img.shape[0]
    endw = img.shape[1]
    endh = img.shape[2]
...
def prepare_data_real_world(dataset_name, data_path,patch_size, stride=1):
    # train
    print('process training data')
    mat_files = open(os.path.join(data_path,dataset_name),'r').readlines()
    ...
    for line in mat_files:
        target_file = "." + line.split(' ')[1][:-1]
        input_file = "." + line.split(' ')[0]
        target = resize(cv2.imread(os.path.join(target_file)))
        b, g, r = cv2.split(target)
        target = cv2.merge([r, g, b])
        target_img = target
        target_img = np.float32(normalize(target_img))
        ...
        input_img = resize(cv2.imread(os.path.join(input_file)))
        b, g, r = cv2.split(input_img)
        input_img = cv2.merge([r, g, b])
        input_img = np.float32(normalize(input_img))
        input_patches = Im2Patch(input_img.transpose(2, 0, 1), win=patch_size, stride=stride)
...
if __name__ == "__main__":
    dataset_path = "real_world.txt"
    prepare_data_real_world(dataset_path,'training',100)
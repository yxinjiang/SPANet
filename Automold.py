...
err_rain_slant="Numeric value between -45 and 45 is allowed"
...
def rain_process(image,slant,drop_length,drop_color,drop_width,rain_drops,blur_kernel=3,brightness_coefficient=0.9):
    image_t= image.copy()
    rain_layer = np.zeros(image.shape, dtype=np.uint8)
    for rain_drop in rain_drops:
        cv2.line(image_t,(rain_drop[0],rain_drop[1]),(rain_drop[0]+slant,rain_drop[1]+drop_length),drop_color,drop_width)
        cv2.line(rain_layer,(rain_drop[0],rain_drop[1]),(rain_drop[0]+slant,rain_drop[1]+drop_length),drop_color,drop_width)

    image= cv2.blur(image_t,(blur_kernel,blur_kernel)) ## rainy view are blurry
    rain_layer = cv2.blur(rain_layer,(blur_kernel,blur_kernel))
    #brightness_coefficient = 0.9 ## rainy days are usually shady 
    image_HLS = hls(image) ## Conversion to HLS
    image_HLS[:,:,1] = image_HLS[:,:,1]*brightness_coefficient ## scale pixel values down for channel 1(Lightness)
    rain_layer_HLS = hls(rain_layer)
    rain_layer_HLS[:,:,1] = rain_layer_HLS[:,:,1]*brightness_coefficient
    #image_HLS = hls(image_t)
    image_RGB= rgb(image_HLS,'hls') ## Conversion to RGB
    rain_layer_RGB = rgb(rain_layer_HLS,'hls')
    return image_RGB, rain_layer_RGB
...
def add_rain(image,slant=-1,drop_length=20,drop_width=1,drop_color=(200,200,200),rain_type='None'): ## (200,200,200) a shade of gray
    verify_image(image)
    slant_extreme=slant
    if not(is_numeric(slant_extreme) and (slant_extreme>=-45 and slant_extreme<=45)or slant_extreme==-1):
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            output, rain_streak= rain_process(img,slant_extreme,drop_length,drop_color,drop_width,rain_drops)
            image_RGB.append(output)
            rain_streak_RGB.append(rain_streak)
    else:
        imshape = image.shape
        if slant_extreme==-1:
            slant= np.random.randint(-10,10) ##generate random slant if no slant value is given
        rain_drops,drop_length= generate_random_lines(imshape,slant,drop_length,rain_type)
        output,rain_streak= rain_process(image,slant_extreme,drop_length,drop_color,drop_width,rain_drops)
        image_RGB=output
        rain_streak_RGB = rain_streak

    return image_RGB,rain_streak_RGB
